...
def get_arguments():
    ...
    parser.add_argument(
        "--model", type=str, default=MODEL, help="available options : DeepLab/DRN"
    )
    parser.add_argument(
        "--dataset", type=str, default=DATASET, help="dataset to be used"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        "--num-steps", type=int, default=NUM_STEPS, help="Number of iterations."
    )
    parser.add_argument(
        "--power",
        type=float,
...
def adjust_threshold_st(i_iter):
    return 0.5 + (args.num_steps - i_iter) * 0.2 / args.num_steps


def one_hot(label):
    label = label.numpy()
    one_hot = np.zeros(
        (label.shape[0], args.num_classes, label.shape[1], label.shape[2]),
        dtype=label.dtype,
    )
...
def find_good_maps(D_outs, pred_all):
    ...
    if count > 0:
        ...
        pred_sel = torch.Tensor(
            count, pred_all.size(1), pred_all.size(2), pred_all.size(3)
        )
...
def main():
    ...
    if args.dataset == "pascal_voc":
        ...
        input_transform = transform.Compose(
            [
                transform.ToTensor(),
                transform.Normalize([0.406, 0.456, 0.485], [0.229, 0.224, 0.225]),
            ]
        )
        ...
        train_dataset = data_loader(
            data_path, split="train", mode="train", **data_kwargs
        )
        ...
        data_aug = Compose(
            [RandomCrop_city((input_size[0], input_size[1])), RandomHorizontallyFlip()]
        )
        train_dataset = data_loader(
            data_path,
            is_transform=True,
    ...
    if args.labeled_ratio is None:
        ...
        pickle.dump(
            train_ids,
            open(os.path.join(args.checkpoint_dir, "train_voc_split.pkl"), "wb"),
        )
    ...
    if args.dataset == "pascal_voc":
        ...
        val_dataset = data_loader(
            data_path, img_size=(512, 1024), is_transform=True, split="val"
        )
        valloader = data.DataLoader(
            val_dataset, batch_size=args.batch_size, shuffle=False, pin_memory=True
        )
        interp_val = nn.Upsample(size=(512, 1024), mode="bilinear", align_corners=True)
    elif args.dataset == "ade20k":
    ...
    scheduler = CosineAnnealingLR(
        ...
        eta_min=args.learning_rate / args.eta_min_factor,
    )
    optimizer.zero_grad()
    ...
    scheduler_D = CosineAnnealingLR(
        optimizer_D,
        T_max=args.num_steps,
        eta_min=args.learning_rate_D / args.eta_min_factor,
    )
    ...
    y_real_, y_fake_ = Variable(torch.ones(args.batch_size, 1).cuda()), Variable(
        torch.zeros(args.batch_size, 1).cuda()
    )
    ...
    for i_iter in range(args.num_steps):
        ...
        images_gt = (images_gt - torch.min(images_gt)) / (
        ...
        if (
            count > 0 and i_iter > 0
        ):  # if any good predictions found for self-training loss
        ...
        if i_iter % 10 == 0:
            ...
            args.writer.add_scalar("train/1.train_loss_ce", losses_ce.avg, log_idx)
            args.writer.add_scalar("train/2.train_loss_st", losses_st.avg, log_idx)
            args.writer.add_scalar("train/3.train_loss_fm", losses_fm.avg, log_idx)
            args.writer.add_scalar("train/4.train_loss_S", losses_S.avg, log_idx)
            args.writer.add_scalar("train/5.train_loss_D", losses_D.avg, log_idx)
            args.writer.add_scalar("train/6.count", counts.avg, log_idx)
            args.writer.add_scalar(
                "train/7.lr", optimizer.param_groups[0]["lr"], log_idx
            )
        ...
        if i_iter >= args.num_steps - 1:
            ...
            torch.save(
                model.state_dict(),
                os.path.join(
                    args.checkpoint_dir, "VOC_" + str(args.num_steps) + ".pth"
                ),
            )
            torch.save(
                model_D.state_dict(),
                os.path.join(
                    args.checkpoint_dir, "VOC_" + str(args.num_steps) + "_D.pth"
                ),
            )
            break
        ...
        if i_iter % args.save_pred_every == 0 and i_iter != 0:
            ...
            torch.save(
                model.state_dict(),
                os.path.join(args.checkpoint_dir, "VOC_" + str(i_iter) + ".pth"),
            )
            torch.save(
                model_D.state_dict(),
                os.path.join(args.checkpoint_dir, "VOC_" + str(i_iter) + "_D.pth"),
            )
    end = timeit.default_timer()
    print(end - start, "seconds")


def validate(valloader, interp_val, model):
    print("validating...")
    loss_val = 0
    data_list = []
    model.eval()
