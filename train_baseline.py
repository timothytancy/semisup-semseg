...
def main():
    ...
    if args.dataset == "pascal_voc":
        ...
        valloader = data.DataLoader(
            VOCDataSet(
                args.data_dir, args.data_list, crop_size=(505, 505), mean=IMG_MEAN, scale=False,
            ),
            batch_size=1,
            shuffle=False,
            pin_memory=True,
    ...
    for i_iter in range(args.num_steps):
        ...
        logging.info(
            "iter = {0:8d}/{1:8d}, loss_seg = {2:.3f}".format(i_iter, args.num_steps, loss.item())
        )
        ...
        if i_iter >= args.num_steps - 1:
            ...
            torch.save(
                model.state_dict(),
                osp.join(args.checkpoint_dir, "VOC_baseline" + str(args.num_steps) + ".pth"),
            )
            break
        ...
        if i_iter % args.save_pred_every == 0 and i_iter != 0:
            ...
            torch.save(
                model.state_dict(),
                osp.join(args.checkpoint_dir, "VOC_baseline" + str(i_iter) + ".pth"),
            )
